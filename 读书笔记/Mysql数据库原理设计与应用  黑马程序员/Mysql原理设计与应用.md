...
1. 创建索引
   ...
   - 全局索引
     ...
     ![image-20231129105219975](assets/image-20231129105219975.png)
   ...
   - 复合索引
     ...
     ![image-20231129125638129](assets/image-20231129125638129.png)
...
2. 查看索引
   ...
   - 查询表中所有索引信息
     ...
     Sub_part：如果索引只是列的一部分，该字段表示索引的长度。
     Packed：指示索引是否使用了压缩格式。
     Null：表示索引列是否允许为空。值为 "YES" 表示允许为空，值为 "NO" 表示不允许为空。
     Index_type：索引的类型，例如 B-tree、Hash 等。
     Comment：索引的注释。
     Index_comment：索引的注释或其他额外信息。
     ```

     ![image-20231129132233250](assets/image-20231129132233250.png)
   ...
   - 分析语句执行情况
     ...
     ```sql
     explain select * from employee where id=10\G;
     ...
     ref：连接条件所使用的列或常数。
     rows：估计的返回行数。
     Extra：额外的信息，例如排序、临时表等。
     ```
   
     ![image-20231129132702379](assets/image-20231129132702379.png)
...
3. 删除索引
   ...
   - 删除主键索引
     ...
     ```sql
     删除主键索引，需要考虑主键字段是否有auto_increment属性，有的话先删除该属性
     
     1.删除主键字段的auto_increment属性
     alter table 数据表 modify 字段名 字段类型
     2.删除主键索引
     alter table 数据表 drop primary key 
     或
     drop index `primary` on 数据表
     主键没有索引名，反引号包裹关键字primary
     ```

     ![image-20231129134902797](assets/image-20231129134902797.png)
...
- 删除非主键索引
  ...
  ![image-20231129135438425](assets/image-20231129135438425.png)
...
- 使用索引的注意点
  ...
  2. 模糊查询通配符的使用
     ```sql
     匹配模式中最左则含有通配符%，会使用全表扫描，而不使用设置的索引
     
     where name like '%笔记%' 使用全表扫描的方式
     where name like '笔记%'  使用复合索引
     ```
